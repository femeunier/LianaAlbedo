...
files <- c("Figure1_Guzman.rds","Figure1_kalacska.rds",
           "Figures5and6_sanchez2009_PNM","Figures5and6_sanchez2009_FTS",
...
# files <- c("Figures5and6_sanchez2009_FTS","Figures5and6_sanchez2009_PNM","Figure1_Guzman.rds")
# Names <- c("Sanchez_FTS","Sanchez_PNM","Guzman")
...
for (ifile in seq(files)){
  ...
  # Plot
  PDA_results <- PDA
  PDA_all[[Names[ifile]]][[Names[ipft]]] <- PDA
  ...
  for (ipft in seq(npft)){
    ...
    model_OP_ensemble_spectrum <- model_OP_ensemble_PAR <- model_OP_ensemble_NIR <-
      model_OP_ensemble_SIR <- model_OP_ensemble_IR <- c()
    ...
    for (irun in seq(nrow(posteriorMat))){
      ...
      model_OP_ensemble_SIR[irun] <- mean(interp1(x =  400:2500,
                                                  y =  as.vector(current_model_output[,1]),
                                                  xi = 1500:2500))
      
      model_OP_ensemble_IR[irun]  <- mean(interp1(x =  400:2500,
                                                  y =  as.vector(current_model_output[,1]),
                                                  xi = 800:2500))
    }
    
    lmdf <- as.data.frame(cbind(posteriorMat,model_OP_ensemble_spectrum,model_OP_ensemble_PAR,model_OP_ensemble_NIR,
                                model_OP_ensemble_SIR,model_OP_ensemble_IR))
    ...
    # SIR
    anovobj<-aov(lm(as.formula(paste0("model_OP_ensemble_SIR"," ~ .^2")),data=lmdf))
    allssq<-summary(anovobj)[[1]][,2]
    varn<-names((anovobj)$coefficients)[-1]
    vars <- varn[1:ncol(posteriorMat)]
    ...
    # IR
    anovobj<-aov(lm(as.formula(paste0("model_OP_ensemble_IR"," ~ .^2")),data=lmdf))
    allssq<-summary(anovobj)[[1]][,2]
    varn<-names((anovobj)$coefficients)[-1]
    vars <- varn[1:ncol(posteriorMat)]
...
saveRDS(object = PDA_all,file = "~/data/RTM/All_PDA_leaf.RDS")