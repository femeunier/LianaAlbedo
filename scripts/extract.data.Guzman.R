...
saveRDS(object = df.data.all_sum %>% filter(site == "PNM") %>% rename(wavelength = wv,
        ...
        file = "~/data/RTM/Figure6_sanchez2009_PNM_all.rds")

saveRDS(object = df.data.all_sum %>% filter(site == "FTS") %>% rename(wavelength = wv,
                                                                      Reflectance = R) %>% mutate(pft = case_when(GF == "Liana" ~ "Liana_optical",
                                                                                                                  GF == "Tree" ~ "Tree_optical")) %>%
          ungroup() %>% dplyr::select(-c(Rmin,Rmax,site,GF)) %>% filter(!(wavelength > 680 & wavelength < 750)),
        file = "~/data/RTM/Figure6_sanchez2009_FTS_all.rds")
