...
# saveRDS(object = PDA_all,file = "~/data/RTM/Inverse_leaf_all.RDS")
...
subplota %>% filter(wavelength<700) %>%group_by(pft) %>% summarise(m = mean(median))
...
subplotA <-
  ...
  # geom_line() +
  geom_point(data = data_subplota  %>% mutate(wavelength = as.numeric(wavelength)) %>% mutate(group_wd = round(wavelength/3)) %>% group_by(ref,group_wd) %>% sample_n(1),
            aes(x = wavelength,
...
# subplota %>% group_by(pft) %>% summarise(NDVI = (mean(median[wavelength>750 & wavelength<900]) - mean(median[wavelength>550 & wavelength<700]))/(mean(median[wavelength>750 & wavelength<900]) + mean(median[wavelength>550 & wavelength<700])))
...
All_leaf_spectra <- All_leaf_spectra %>% mutate(wavelength=as.integer(wavelength))
ready.for.plot <- All_leaf_spectra %>%left_join(posterior,by=c("ref","pft","wavelength")) %>% filter(!(ref %in% c("Sanchez_PNM","Sanchez_FTS") &wavelength>680 & wavelength<750))
...
subplotC <-
  ...
  geom_point(data = data_subplotc %>% mutate(group_wd = round(wavelength/2)) %>% group_by(ref,group_wd) %>% sample_n(1),
            aes(x = wavelength,
                y = Reflectance_median,
                color = scenario),size=1) + 
  scale_color_manual(values = c("#1E64C8","#137300")) +
  scale_fill_manual(values = c("#1E64C8","#137300")) +
  scale_y_continuous(limits = c(0,0.6),expand = c(0.001, 0.001)) +
  scale_x_continuous(expand = c(0.001, 0.001)) +
  theme_bw() + 
  labs(x = "Wavelength [nm]",
  ...
  theme(axis.text = element_text(size=14),
        axis.title = element_text(size=12),
        axis.title.x = element_text(margin = margin(t = 5, r = 0, b = 0, l = 0)),
        axis.title.y = element_text(margin = margin(t = 0, r = 5, b = 0, l = 0))) + theme(legend.position = c(0.8,0.8)) +
...
subplotD <-
  ...
  scale_y_continuous(limits = c(0,0.65),expand = c(0.001, 0.001),breaks = seq(0,0.6,0.2)) +
  scale_x_continuous(limits = c(0,0.65),expand = c(0.001, 0.001),breaks = seq(0,0.6,0.2)) +
  scale_shape_manual(values=c(2,15,17,16)) +
  geom_abline(slope = 1, intercept = 0,colour = 'black',linetype=2) + 
  theme_bw() + 
  theme(axis.text = element_text(size=14),
        axis.title = element_text(size=12),
