...
########################################################################################
# Sanchez 2009 with transmittance
rm(list=ls())
...
wl.min <- 450
wl.max <- 950
data.all <- read.csv("~/data/RTM/data/Figures5and6_Sanchez.csv") %>% rename(GF = pft) %>% filter(wl <= wl.max,
                                                                                                 wl >= wl.min)
...
for (isite in seq(sites)){
  ...
  datatemp <- data.all %>% filter(Site == sites[isite],
                                  type == "Reflectance") %>% mutate(GF = case_when(
                                    GF == "Tree" ~ "Tree_optical",
                                    GF == "Liana" ~ "Liana_optical"))
  datatempT <- data.all %>% filter(Site == sites[isite],
                                   type == "Transmittance") %>% mutate(GF = case_when(
                                     GF == "Tree" ~ "Tree_optical",
                                     GF == "Liana" ~ "Liana_optical"))
  for (ipft in seq(PFTs)){
    ...
    temp <- datatemp %>% filter(GF == pft) %>% dplyr::select(wl,value) %>% rename(R = value)
    tempT <- datatempT %>% filter(GF == pft) %>% dplyr::select(wl,value) %>% rename(T = value)
    
    pos <- sort(temp[,1],index.return = TRUE)
    posT <- sort(tempT[,1],index.return = TRUE)
  
    
    R <- rbind(c(450,temp[1,2]),
               temp[pos$ix,],
               c(950,temp[length(pos$x),2]))
    
    Tr <- rbind(c(450,tempT[1,2]),
                tempT[posT$ix,],
                c(950,tempT[length(posT$x),2]))
    
    R_interp <- interp1(R[,1],R[,2],wl.min:wl.max)
    T_interp <- interp1(Tr[,1],Tr[,2],wl.min:wl.max)
    
    data_R[[pft]] <- cbind(wl.min:wl.max,R_interp)
    data_T[[pft]] <- cbind(wl.min:wl.max,T_interp)
    
    lines(data_T[[pft]] [,1],data_T[[pft]] [,2],col = C[ipft],type='l')
    names(data_R[[pft]]) <- c("wavelength","reflectance")
    
    df <- rbind(df,data.frame(wavelength =  data_R[[pft]] [,1],
                              Reflectance = data_R[[pft]] [,2],
                              Transmittance = data_T[[pft]][,2],
                              pft = pft))
  }
  saveRDS(df,file = paste0("~/data/RTM/Figures5and6_sanchez2009_",sites[isite],".rds"))
}

# ggplot(data = data.all %>% filter(type == "Transmittance"))+
#   geom_line(aes(x = wl,y = value,color = GF)) +
#   facet_wrap(~ Site)
# 
# rm(list=ls())
# 
...
#     if (isite == 2 & ipft == 1) data_R[[pft]] [,1] = data_R[[pft]] [,1]-10
#     lines(data_R[[pft]] [,1],data_R[[pft]] [,2],col = C[ipft])
#     names(data_R[[pft]]) <- c("wavelength","reflectance")
...
All_leaf_spectra <-
  rbind(readRDS(file = "~/data/RTM/Figure1_Guzman.rds") %>% mutate(ref = "Guzman"),
      ...
      readRDS(file = "~/data/RTM/Figure6_sanchez2009_PNM_all2.rds") %>% mutate(ref = "Sanchez_PNM"),
      readRDS(file = "~/data/RTM/Figure6_sanchez2009_FTS_all2.rds") %>% mutate(ref = "Sanchez_FTS"))
...
All_leaf_spectra_diff <- All_leaf_spectra2 %>% group_by
ggplot(data=All_leaf_spectra2,
       aes(x = wavelength[pft == "Liana"],
           y = Reflectance_median[pft == "Liana"] - Reflectance_median[pft == "Tree"],
           ymin = Reflectance_min[pft == "Liana"],
           ymax = Reflectance_max[pft == "Liana"],
           group = pft[pft == "Liana"],
           fill = pft[pft == "Liana"],
           color = pft[pft == "Liana"])) +
...
bands_m <- c(550,600,400,800,1500,1500,2000,680)
bands_M <- c(600,800,700,1400,2400,1800,2500,700)
band_names <- c('green','Red edge','visible','NIR',"SWIR_all",'SWIR','SWIR2','Rededge2')
...
for (cref in (ref_alls)){
  ...
  pfts <- as.character(unique(All_leaf_spectra[pos_ref,"pft"] %>% pull(pft)))
  for (cpft in pfts){
    cdata <- All_leaf_spectra %>% dplyr::filter(ref == cref & pft == cpft)
    waves <- cdata %>% pull(wavelength)
   
    R <- interp1(x = waves,y = cdata %>% pull(Reflectance_median),waves_all,extrap = TRUE)
    df.all <- bind_rows(list(df.all,
                             data.frame(pft = as.character(cpft),
                                        ref = as.character(cref),
                                        wl = waves_all,
                                        R = R)))
  }
}

diff <- df.all %>% group_by(wl,ref) %>% summarise(diff_R = R[pft == "Liana_optical"] - R[pft == "Tree_optical"])
